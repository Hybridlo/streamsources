...
use super::super::state::{PredictionState, PredictionStatus};
use crate::front_common::{scalable_wrapper, predictions::PredictionsProps, SourceColor};
...
fn status(status: &PredictionStatus, lock_time: &DateTime<Utc>) -> Html {
    // state needed only for the timer
    let timer_animation: Rc<RefCell<Option<Interval>>> = use_mut_ref(|| None);
    let timer = use_state(|| "--:--".to_string());
    let lock_time_state = use_state_eq(|| lock_time.clone());
    // reset lock time when there's a new one incoming (use_state_eq is used for that reason)
    lock_time_state.set(lock_time.clone());

    match status {
        PredictionStatus::InProgress => {
            let timer_not_running = timer_animation.borrow().is_none();

            if timer_not_running {
                let target_time = lock_time_state.clone();
                let timer = timer.clone();
                let timer_animation_clone = timer_animation.clone();
                timer_animation.replace(Some(Interval::new(1000, move || {
                    let time_now = Utc::now();
                    let time_left = (*target_time - time_now).num_seconds();

                    let minutes = time_left.checked_div(60).unwrap_or_default();
                    let seconds = time_left.checked_rem(60).unwrap_or_default();

                    timer.set(format!("{minutes:0>2}:{seconds:0>2}"));
...
pub fn predictions_list(props: &PredictionsProps) -> Html {
    let outline_color = if props.color == SourceColor::White { "white" } else { "black" };
    let outline_color_opposite = if props.color == SourceColor::White { "black" } else { "white" };
